...
wget https://github.com/openssl/openssl/archive/OpenSSL_${OPENSSL_VERSION}.tar.gz >>"${main_log}" 2>>"${err_log}"
	ERROR=$?
	if [[ "$ERROR" != '0' ]]; then
      echo "Error: OpenSSL_${OPENSSL_VERSION}.tar.gz download failed."
      exit
    fi

tar -xzf OpenSSL_${OPENSSL_VERSION}.tar.gz >>"${main_log}" 2>>"${err_log}"
	ERROR=$?
	if [[ "$ERROR" != '0' ]]; then
      echo "Error: OpenSSL_${OPENSSL_VERSION}.tar.gz is corrupted."
      exit
    fi
