...
check_system() {
	...
	if [ $(lsb_release -is) != 'Debian' ] && [ $(lsb_release -is) != 'Ubuntu' ]; then
		echo "${error} The script for now works only on $(textb Ubuntu) $(textb 16.04 Xenial) and $(textb Debian) $(textb 9.x)"
		exit 1
	fi
	...
	if [ $(grep MemTotal /proc/meminfo | awk '{print $2}') -lt 1000000 ]; then
		echo "${warn} This script needs at least ~1000MB of memory"
	fi
	
	FREE=`df -k --output=avail "$PWD" | tail -n1`  
	if [[ $FREE -lt 8388608 ]]; then
		echo "${error} This script needs at least 8 GB free disk space"
		exit 1
	fi

	if [ $(dpkg-query -l | grep dmidecode | wc -l) -ne 1 ]; then
    	echo "${error} This script does not support the virtualization technology!"
		exit 1
	fi
...
#			echo > /dev/null
#		else
#	        echo "${warn} This script does not support the virtualization technology ($(dmidecode -s system-product-name))"
#			exit 1
#       fi
#	fi	
}