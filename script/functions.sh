...
error_exit()
{
	...
	case $CHOICE in
			1)
				...
				#PGP issue
				echo "Here are the error Logs from failed installation ( $USED_OS ) of NeXt Server Installation. Error: $1" | mutt -a "${SCRIPT_PATH}/logs/main.log" "${SCRIPT_PATH}/logs/error.log" "${SCRIPT_PATH}/logs/make.log" "${SCRIPT_PATH}/logs/make_error.log" "${SCRIPT_PATH}/configs/userconfig.cfg" "${SCRIPT_PATH}/logs/addons_error.log" -s "FAILED INSTALLATION OF NeXt Server Installation" -- error@perfectrootserver.de >/dev/null 2>&1
